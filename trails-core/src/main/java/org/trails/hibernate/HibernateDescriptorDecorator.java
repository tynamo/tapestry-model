 ...
 * - ReflectorDescriptorFactory allocates property descriptor instance for 
 *   the class type 
 ...
 *   decorated descriptor list 
 * - decorated descriptor list gets populated into class descriptor for class type 
 * - TrailsDescriptorService finally populates decorated class descriptor and it's
 *   aggregated list of decorated property descriptors into it's own list/cache of
 *   referenced class descriptors
 * 
...
public class HibernateDescriptorDecorator implements DescriptorDecorator {
	...
	public IClassDescriptor decorate(IClassDescriptor descriptor) {
		...
		for (IPropertyDescriptor propertyDescriptor : descriptor
			...
			try {
				...
				if (propertyDescriptor.getName().equals(
					...
					descriptorReference = createIdentifierDescriptor(type,
							propertyDescriptor);
					...
					Property mappingProperty = getMapping(type).getProperty(
							propertyDescriptor.getName());
					descriptorReference = decoratePropertyDescriptor(type,
							mappingProperty, propertyDescriptor, descriptor);
				}
	...
	protected IPropertyDescriptor decoratePropertyDescriptor(Class type,
		...
		IDescriptorExtension descriptorExtension = null;
		...
		if (mappingProperty.getType() instanceof ComponentType) {
			...
			descriptor = embeddedDescriptor;
				...
				.getPropertyType())) {
			descriptorExtension = decorateCollectionDescriptor(type,
					descriptor, parentClassDescriptor);
			descriptor.addExtension(descriptorExtension.getClass().getName(),
					descriptorExtension);
		} else if (hibernateType.isAssociationType()) {
			descriptorExtension = decorateAssociationDescriptor(type,
					mappingProperty, descriptor, parentClassDescriptor);
	...
	public IDescriptorExtension decorateAssociationDescriptor(Class type,
			Property mappingProperty, IPropertyDescriptor descriptor,
			IClassDescriptor parentClassDescriptor) {
		Type hibernateType = mappingProperty.getType();
		type = parentClassDescriptor.getType();

		IDescriptorExtension descriptorReference = null;

		try {
			Field propertyField = null;
			try {
				propertyField = type.getDeclaredField(descriptor.getName());
			} catch (NoSuchFieldException ex) {
				propertyField = type.getSuperclass().getDeclaredField(
						descriptor.getName());
			}
			...
			if (readMethod
				...
				if ("".equals(mappedBy)) {
					...
					descriptorReference = new OwningObjectReferenceDescriptor(
							type, descriptor);
					((OwningObjectReferenceDescriptor) descriptorReference)
							.setOneToOne(true);
					((OwningObjectReferenceDescriptor) descriptorReference)
							.setInverseProperty(ognlUsableProperty);
				} else {
					descriptorReference = new ObjectReferenceDescriptor(type,
							descriptor);
		...
		if (collectionDescriptor.isOneToMany() && collectionMapping.isInverse()) {
			try {
				...
				Field propertyField = type
						.getDeclaredField(collectionDescriptor.getPropertyDescriptor().getName());
				PropertyDescriptor beanPropDescriptor = (PropertyDescriptor) Ognl
						.getValue("propertyDescriptors.{? name == '"
								+ collectionDescriptor.getPropertyDescriptor().getName() + "'}[0]",
